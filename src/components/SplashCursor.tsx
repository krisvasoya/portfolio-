...
class Pointer {
  ...
  down = false;
  moved = false;
  color = [0, 0, 0] as [number, number, number] | { r: number; g: number; b: number } | any;
}
...
function SplashCursor({
  ...
  useEffect(() => {
    ...
    function getWebGLContext(canvasEl: HTMLCanvasElement) {
      ...
      let webglCtx: any = canvasEl.getContext('webgl2', params);
      const isWebGL2 = !!webglCtx;
      if (!isWebGL2) webglCtx = canvasEl.getContext('webgl', params) || canvasEl.getContext('experimental-webgl', params);
      ...
      const halfFloatTexType = isWebGL2 ? webglCtx.HALF_FLOAT : halfFloat && halfFloat.HALF_FLOAT_OES;
      let formatRGBA: any;
      let formatRG: any;
      let formatR: any;
    ...
    function getSupportedFormat(webglCtx: any, internalFormat: number, format: number, type: any) {
      if (!supportRenderTextureFormat(webglCtx, internalFormat, format, type)) {
        switch (internalFormat) {
          case webglCtx.R16F:
            return getSupportedFormat(webglCtx, webglCtx.RG16F, webglCtx.RG, type);
          case webglCtx.RG16F:
            return getSupportedFormat(webglCtx, webglCtx.RGBA16F, webglCtx.RGBA, type);
          default:
            return null;
        }
      }
    ...
    function supportRenderTextureFormat(webglCtx: any, internalFormat: number, format: number, type: any) {
